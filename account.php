...
<df-messenger
  intent="WELCOME"
  chat-title="Helpdesk"
  agent-id="32959454-2cca-4d1c-8fce-f55700e596f6"
  language-code="en"
>
</df-messenger>
    ...
    <div class="collapse navbar-collapse" id="bs-example-navbar-collapse-1">
      <ul class="nav navbar-nav">
		...
		<!-- <li class="pull-right"> <a href="logout.php?q=account.php"><span class="glyphicon glyphicon-log-out" aria-hidden="true"></span>&nbsp;&nbsp;&nbsp;&nbsp;Signout</a></li> -->
            ...
            <form class="navbar-form navbar-left" role="search" action="account1.php" method="post">
        <div class="form-group">
          <input type="text" class="form-control" placeholder="Enter tag "name="t1">
        </div>
        <button type="submit" name="search" class="btn btn-default"><span class="glyphicon glyphicon-search" aria-hidden="true"></span>&nbsp;Search</button>
      </form>
      </div><!-- /.navbar-collapse -->
  </div><!-- /.container-fluid -->
...
<!--<div id="app" style="font-family: sans-serif;
  ...
  place-items: center;"></div>-->
<!--
<div id="previous" style ="font-family: sans-serif;
  display: grid;
  place-items: left;">
<form action="update.php?q=quiz&step=2&eid=.$eid.&n=.$sn.&t=.$total." >
  <button id="previous"  >previous</button></form>
  </div>-->
...
const TOTAL = '<?php echo $timelim; ?>';
console.log(TOTAL);
const TIME_LIMIT ='<?php echo $timelim; ?>';
let timePassed = 0;
...
echo '<tr></tr></table></div><div class="panel"><br /><h3 align="center" style="font-family:calibri">:: Detailed Analysis ::</h3><br /><ol style="font-size:20px;font-weight:bold;font-family:calibri;margin-top:20px">';
        ...
        while ($row = mysqli_fetch_array($q)) {
            $question = $row['qns'];
            $qid      = $row['qid'];
			echo '<div style="font-size:20px;font-weight:bold;font-family:calibri;margin-top:20px;background-color:lightblue;padding:10px;word-wrap:break-word;border:2px solid darkblue;border-radius:10px;">' . $question . ' </div><br />';
           ...
           // if (mysqli_num_rows($q2) > 0) {
                ...
                $q3 = mysqli_query($con, "SELECT * FROM options WHERE qid='$qid'") or die('Error197');
                while($row2=mysqli_fetch_array($q3) )
{
$option=$row2['option'];
//$optionid=$row['optionid'];
//echo'<input type="radio" name="ans" >'.$option.'<br /><br />';
echo '<font style="font-size:20px;color:darkgreen"><b>*</b></font><font style="font-size:20px;">' . $option . '</font><br />';
}
                ...
                $q5 = mysqli_query($con, "SELECT * FROM questions WHERE qid='$qid'") or die('Error197');
                $row5         = mysqli_fetch_array($q5);
                $correctdesc = $row5['details'];
                
           
            
                
                //echo '<font style="font-size:14px;color:darkgreen"><b>Your Answer: </b></font><font style="font-size:14px;">' . $ans . '</font><br />';
                echo '<font style="font-size:22px;color:darkgreen"><b>Correct Answer: </b></font><font style="font-size:22px;">' . $correctans . '</font><br />';
                echo '<font style="font-size:22px;color:brown"><b>Explanation: </b></font><font style="font-size:22px;">' . $correctdesc . '</font><br />';
...
$e=$row['email'];
$s=$row['score'];
$q12=mysqli_query($con,"SELECT * FROM user  " )or die('Error231');
